...
export default async function threadRoutes(app: FastifyInstance) {
  ...
  app.get("/projects/:pid/threads", async (req, reply) => {
    ...
    const project = await app.prisma.project.findFirst({
      where: { id: pid, workspace: { members: { some: { userId: req.userId } } } },
    });
  ...
  app.post("/projects/:pid/threads", async (req, reply) => {
    ...
    const project = await app.prisma.project.findFirst({
      where: {
        id: pid,
        workspace: { members: { some: { userId: req.userId, role: { in: [...EDITOR_ROLES] } } } },
      },
    });
    if (!project) return reply.code(403).send({ error: "Forbidden" });
  ...
  app.get("/threads/:tid", async (req, reply) => {
    ...
    const thread = await app.prisma.chatThread.findFirst({
      where: { id: tid, project: { workspace: { members: { some: { userId: req.userId } } } } },
      include: { _count: { select: { messages: true } } },
    });
    if (!thread) return reply.code(404).send({ error: "Not found" });
    return { ...thread, messageCount: thread._count.messages };
  });
